function html(){
  ...
  function getCompiledCodeFilename(){
    ...
    try {
      ...
      var idx = strongName.indexOf($intern_65);
      if (idx != -1) {
    ...
    return computeUrlForResource(strongName + $intern_66);
  }
  ...
  function loadExternalStylesheets(){
    ...
    function installOneStylesheet(stylesheetUrl){
      if (!__gwt_stylesLoaded[stylesheetUrl]) {
        ...
        l.setAttribute($intern_70, computeUrlForResource(stylesheetUrl));
        $doc.getElementsByTagName($intern_71)[$intern_7].appendChild(l);
  ...
  if ($wnd) {
    ...
    if (devModePermitted && supportsSessionStorage()) {
      ...
      if (devModeUrl && !$wnd[devModeKey]) {
        ...
        var head = $doc.getElementsByTagName($intern_71)[$intern_7];
